...
describe('Lambda Handler', () => {
    ...
    describe('handler', () => {
        it('should process single SQS message with both Identify and ListRecords phases', async () => {
            ...
            mockOaiProcessor.processListRecords.mockImplementation(async (url, journalKey, pageCallback) => {
                await pageCallback('<OAI-PMH><ListRecords><record>test record</record></ListRecords></OAI-PMH>', 1, 1, 1);
                return {
                    pageCount: 1,
            ...
            expect(mockOaiProcessor.processIdentify).toHaveBeenCalledWith('https://example.com/oai', 'test-journal-123');
        ...
        it('should handle multiple SQS messages', async () => {
            ...
            mockS3Processor.createAndUploadXml.mockResolvedValue('https://test-bucket.s3.amazonaws.com/test.xml');
        ...
        it('should handle ListRecords phase failure', async () => {
            ...
            mockS3Processor.createAndUploadXml.mockResolvedValue('https://test-bucket.s3.amazonaws.com/identify.xml');
            mockSqsProcessor.sendMessage.mockResolvedValue('message-id');
