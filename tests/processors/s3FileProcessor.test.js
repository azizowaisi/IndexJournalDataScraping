...
describe('S3FileProcessor', () => {
    ...
    describe('constructor', () => {
        ...
        it('should throw error if S3_BUCKET_NAME is not set', () => {
            delete process.env.S3_BUCKET_NAME;
            expect(() => new S3FileProcessor()).toThrow('S3_BUCKET_NAME environment variable is required');
        });
    });
    ...
    describe('createXmlFileForRecord', () => {
        it('should create well-formed XML for individual record', () => {
            ...
            const result = processor.createXmlFileForRecord(recordXml, journalKey, oaiUrl, recordNumber, pageNumber);
        ...
        it('should handle empty record data', () => {
            const result = processor.createXmlFileForRecord('', 'test-journal', 'https://example.com/oai', 1, 1);
        ...
        it('should escape XML special characters', () => {
            ...
            const result = processor.createXmlFileForRecord(recordXml, 'test<journal>', 'https://example.com/oai', 1, 1);
    ...
    describe('generateS3KeyForRecord', () => {
        it('should generate S3 key with date-based folder structure', () => {
            ...
            expect(result).toMatch(/^oai-data\/2024-01-15\/test-journal\/test-journal-record-5-page-2-1705312200000\.xml$/);
        });
    ...
    describe('createAndUploadXmlForRecord', () => {
        it('should create XML and upload to S3 for individual record', async () => {
            ...
            const result = await processor.createAndUploadXmlForRecord(recordXml, journalKey, oaiUrl, recordNumber, pageNumber);

            expect(result).toMatch(/^https:\/\/test-bucket\.s3\.us-east-1\.amazonaws\.com\/oai-data\/\d{4}-\d{2}-\d{2}\/test-journal\/test-journal-record-1-page-1-\d+\.xml$/);
        ...
        it('should handle S3 upload errors for individual record', async () => {
            ...
            await expect(
                processor.createAndUploadXmlForRecord('<record>test</record>', 'test-journal', 'https://example.com/oai', 1, 1)
    ...
    describe('uploadToS3', () => {
        ...
        it('should handle S3 upload errors', async () => {
            ...
            await expect(processor.uploadToS3('test/key.xml', 'content'))
                .rejects.toThrow('Failed to upload to S3: Access denied');
        });
    });
    ...
    describe('createAndUploadXml (legacy method)', () => {
        it('should create XML and upload to S3 successfully', async () => {
            ...
            expect(result).toMatch(/^https:\/\/test-bucket\.s3\.us-east-1\.amazonaws\.com\/oai-data\/test-journal-\d+\.xml$/);
            expect(mockS3Client.send).toHaveBeenCalledTimes(1);
        });
    ...
    describe('escapeXml', () => {
        it('should escape XML special characters', () => {
            expect(processor.escapeXml('Test & "Special" Characters')).toBe('Test &amp; &quot;Special&quot; Characters');
