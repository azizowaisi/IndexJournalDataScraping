...
describe('OaiDataProcessor', () => {
  ...
  describe('processIdentify', () => {
    ...
    it('should validate OAI URL format', async () => {
      await expect(processor.processIdentify('invalid-url', 'test-journal')).resolves.toEqual(
    ...
    it('should require non-empty OAI URL', async () => {
      await expect(processor.processIdentify('', 'test-journal')).resolves.toEqual(
        expect.objectContaining({
          success: false,
          errorCode: expect.any(String),
    ...
    it('should process ListRecords request with pagination', async () => {
      ...
      parseStringPromise
        .mockResolvedValueOnce({
          'OAI-PMH': {
            ListRecords: {
              record: [{ header: { identifier: 'record1' }, metadata: { title: 'Test 1' } }],
        ...
        .mockResolvedValueOnce({
          'OAI-PMH': {
            ListRecords: {
              record: [{ header: { identifier: 'record2' }, metadata: { title: 'Test 2' } }],
            },
          },
        });
      ...
      const result = await processor.processListRecords(
      ...
      expect(mockAxios).toHaveBeenNthCalledWith(
        1,
        'https://example.com/oai?verb=ListRecords&metadataPrefix=oai_dc',
        expect.any(Object)
      );
      expect(mockAxios).toHaveBeenNthCalledWith(
        2,
        'https://example.com/oai?verb=ListRecords&resumptionToken=token123',
        expect.any(Object)
      );
    });
  ...
  describe('URL building methods', () => {
    ...
    it('should handle URLs with trailing slashes', () => {
      expect(processor.buildIdentifyUrl('https://example.com/oai/')).toBe(
        'https://example.com/oai?verb=Identify'
      );
      expect(processor.buildListRecordsUrl('https://example.com/oai/')).toBe(
        'https://example.com/oai?verb=ListRecords&metadataPrefix=oai_dc'
      );
    });
  });
  ...
  describe('error code mapping', () => {
    ...
    it('should map Axios errors correctly', () => {
      expect(processor.getErrorCode({ name: 'AxiosError', code: 'ECONNABORTED' })).toBe(
        'REQUEST_TIMEOUT'
      );
      expect(processor.getErrorCode({ name: 'AxiosError', code: 'ERR_NETWORK' })).toBe(
        'NETWORK_ERROR'
      );
    });
