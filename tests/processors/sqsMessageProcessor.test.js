...
describe('SqsMessageProcessor', () => {
    ...
    describe('constructor', () => {
        it('should initialize with SQS queue URL from environment', () => {
            expect(processor.integrationQueueUrl).toBe('https://sqs.us-east-1.amazonaws.com/123456789012/test-integration-queue');
            expect(mockSQSClient).toBeDefined();
        });

        it('should throw error if SQS_INTEGRATION_QUEUE_URL is not set', () => {
            delete process.env.SQS_INTEGRATION_QUEUE_URL;
            expect(() => new SqsMessageProcessor()).toThrow('SQS_INTEGRATION_QUEUE_URL environment variable is required');
        });
    });
    ...
    describe('sendRecordMessage', () => {
        ...
        it('should handle SQS send errors for record message', async () => {
            ...
            await expect(processor.sendRecordMessage(messageData))
                .rejects.toThrow('Failed to send record message to SQS: SQS send failed');
        });
    });
    ...
    describe('sendMessageToQueue', () => {
        ...
        it('should handle SQS errors', async () => {
            ...
            await expect(processor.sendMessageToQueue(queueUrl, messageData))
                .rejects.toThrow('Failed to send message to SQS: SQS error');
        });
    });
    ...
    describe('sendMessage (legacy method)', () => {
        ...
        it('should handle SQS send errors', async () => {
            ...
            await expect(processor.sendMessage(messageData))
                .rejects.toThrow('Failed to send message to SQS: SQS send failed');
        });
    });
});