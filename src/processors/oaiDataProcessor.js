...
class OaiDataProcessor {
    ...
    async processIdentify(oaiUrl, journalKey) {
        console.log(`Phase 1: Processing Identify request for journal: ${journalKey} from URL: ${oaiUrl}`);
        ...
        try {
            ...
            console.log(`Successfully received Identify response with ${response.data.length} characters`);
    ...
    async processListRecords(oaiUrl, journalKey, pageCallback) {
        console.log(`Phase 2: Processing ListRecords request for journal: ${journalKey} from URL: ${oaiUrl}`);
        ...
        try {
            ...
            do {
                ...
                if (result && result['OAI-PMH'] && result['OAI-PMH'].ListRecords) {
                    ...
                    console.log(`Processed page ${pageCount} with ${recordsInPage} records. Total processed: ${totalRecordsProcessed}`);
            ...
            console.log(`ListRecords processing completed. Total pages: ${pageCount}, Total records processed: ${totalRecordsProcessed}`);
    ...
    extractResumptionTokenFromParsed(listRecords) {
        try {
            if (listRecords.resumptionToken) {
                console.log('Found resumptionToken in parsed data:', JSON.stringify(listRecords.resumptionToken, null, 2));
