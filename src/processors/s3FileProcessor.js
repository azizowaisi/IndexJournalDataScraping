...
class S3FileProcessor {
    ...
    createXmlFileForRecord(recordXml, journalKey, oaiUrl, recordNumber, pageNumber) {
        try {
            ...
            console.log(`Created XML file for record ${recordNumber} with ${xmlContent.length} characters`);
            
            return xmlContent;
            
        } catch (error) {
            console.error(`Failed to create XML file for record ${recordNumber} of journal:`, journalKey, error);
    ...
    async createAndUploadXmlForRecord(recordXml, journalKey, oaiUrl, recordNumber, pageNumber) {
        try {
            const xmlContent = this.createXmlFileForRecord(recordXml, journalKey, oaiUrl, recordNumber, pageNumber);
            const s3Key = this.generateS3KeyForRecord(journalKey, recordNumber, pageNumber);
            const s3Url = await this.uploadToS3(s3Key, xmlContent);
            
            console.log(`Successfully created and uploaded XML file for record ${recordNumber} to S3: ${s3Url}`);
            return s3Url;
            
        } catch (error) {
            console.error(`Failed to create and upload XML file for record ${recordNumber} of journal: ${journalKey}`, error);
            throw new Error(`Failed to create and upload XML file for record ${recordNumber}: ${error.message}`);
        }
    }
