...
class SqsMessageProcessor {
    ...
    async sendRecordMessage(messageData) {
        try {
            ...
            console.log(`Successfully sent record message to integration queue for record ${messageData.recordNumber}:`, messageId);
            
            return messageId;
            
        } catch (error) {
            console.error(`Failed to send record message for record ${messageData.recordNumber} of journal:`, messageData.journalKey, error);
            throw error;
        }
    }
