...
exports.handler = async (event) => {
    ...
    for (const record of event.Records) {
        try {
            ...
            if (!url || !journalKey) {
                console.error('Missing required fields in SQS message. URL:', url, 'JournalKey:', journalKey);
                continue;
            }
            ...
            try {
                ...
                if (identifyResult.success) {
                    ...
                    const identifyS3Result = await s3Processor.createAndUploadXml(identifyResult.data, `${journalKey}-identify`, url);
            ...
            try {
                const listRecordsResult = await oaiProcessor.processListRecords(url, journalKey, async (pageXml, pageNumber, recordsInPage, recordsProcessed) => {
                    ...
                    try {
                        ...
                        const pageS3Result = await s3Processor.createAndUploadXml(pageXml, `${journalKey}-listrecords-page-${pageNumber}`, url);
                ...
                if (!listRecordsResult.success) {
                    ...
                    console.log(`Successfully processed ListRecords phase for journal: ${journalKey} - ${listRecordsResult.totalRecordsProcessed} records across ${listRecordsResult.pageCount} pages`);
                }
